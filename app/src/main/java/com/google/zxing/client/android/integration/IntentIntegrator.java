...
//===============KYT 두번째 버튼 사진 Image Code 실행시 
  public static AlertDialog initiateScan2(Activity activity) {
	    return initiateScan2(activity, DEFAULT_TITLE, DEFAULT_MESSAGE, DEFAULT_YES, DEFAULT_NO);
	  }
//===============KYT 두번째 버튼 사진 Image Code 실행시 
  /**
   * See {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence)} --
...
ALL_CODE_TYPES);
  ...
  public static AlertDialog initiateScan2(Activity activity,
          ...
          CharSequence stringButtonNo,
          CharSequence stringDesiredBarcodeFormats) {
Intent intentScan = new Intent("com.google.zxing.client.android.SCAN2"); //실제 수행 액티비티는 menifest파일 에 정의되어 있따. 
intentScan.addCategory(Intent.CATEGORY_DEFAULT);
...
try {
activity.startActivityForResult(intentScan, REQUEST_CODE); //요놈이 결국 스캔 시작 액티비티 
return null;
} catch (ActivityNotFoundException e) {
return showDownloadDialog(activity, stringTitle, stringMessage, stringButtonYes, stringButtonNo);
}
